...
void solve(){
    ...
    for (ll  i = 0; i < q ; i++)
    {
        ...
        for (ll  i = 0; i < m ; i++)
        {
            ...
            rr--;
            if(oo=='>'){
                ll che=upper_bound(a[rr].begin(),a[rr].end(),cc)-a[rr].begin();
                low=max(low,che);
                // cout << che << " = che , low = " << low << endl;
            }
            else{
                ll che=lower_bound(a[rr].begin(),a[rr].end(),cc)-a[rr].begin();
                hig=min(hig,che-1);
                // cout << che << " = che , hig = " << hig << endl;
            }
        }
        if(low>hig){
            cout << -1 << endl;
