...
vector<ll> a,blck,siz;
ll sol(ll ind){
    if(ind==0)return 0;
    ll lb=ind;
    auto che=lower_bound(siz.begin(),siz.end(),ind);
    ll bno=che-siz.begin();
    if(che!=siz.begin()){che--;
        lb=ind-(*che);
    }
    ll res=0;
    for (ll  i = 0; i < bno ; i++)
...
void solve(){
    ...
    cin>>m;
    blck[n-1]=a[n-1];siz[0]=n;
    for (ll  i = n-2; i >=0 ; i--)
    {
        siz[n-i-1]=siz[n-i-2]+i+1;
        blck[i]=blck[i+1]+((a[i])*(n-i));
    }
